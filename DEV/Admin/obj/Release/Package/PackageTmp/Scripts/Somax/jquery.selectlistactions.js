...
(function ($) {
    ...
    $.fn.moveToList = function (sourceList, destinationList, selectedOption) {
        ...
        if (sourceArray.length > 0)
            $(destinationList).append(GetHtml($(opts).val(), $(opts).text(), $(opts).attr('disabled') ? "disabled" : "", $(opts).data('order')));
        //}

    };

    function GetHtml(dval, dtext, dDisabled, order) {
        return "<li class='" + dDisabled + "' data-val='" + dval + "'><span>" + dtext + "</span></li>";
    }
