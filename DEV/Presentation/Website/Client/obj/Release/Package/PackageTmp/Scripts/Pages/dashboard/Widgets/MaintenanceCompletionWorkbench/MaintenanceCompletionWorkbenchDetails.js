...
$(document).on('click', "#emergencyOnDemandPopUpPageID", function (e) {
    ...
    e.preventDefault();
    GoOnDemand(workOrderId, clientLookupId);
    $('#UnplannedWo').modal('hide');
    $('.modal-backdrop').remove();
});
function GoOnDemand(workOrderID, clientLookupId) {
    $.ajax({
        url: "/Dashboard/AddOnDemandWO",
        type: "POST",
        dataType: "html",
...
$(document).on('click', '.btnCancelOnDemand', function () {
    $("#AddOnDemandWOModalpopup").modal('hide');
    //StatusArray = [];
    var areaChargeToId = "";
...
function GoEmergencyDescribe(workOrderID, clientLookupId) {
    $.ajax({
        //url: "/Dashboard/AddDescribeWO",
        url: "/Dashboard/AddDescribeWODynamic",
        type: "POST",
        dataType: "html",
        data: { WorkoderId: workOrderID, ClientLookupId: clientLookupId },
        beforeSend: function () {
        ...
        success: function (data) {
            //$('#DescribeWOPopUp').html(data);
            //$('#AddDescribeWOModalpopup').modal({ backdrop: 'static', keyboard: false, show: true });
            $('#DescribeWOPopUpDynamic').html(data);
            $('#AddDescribeWOModalpopupDynamic').modal({ backdrop: 'static', keyboard: false, show: true });
        },
        complete: function () {
            SetControls();
            $("#imgChargeToTree").css("display", "none");
...
function generateLaborGrid() {
    ...
    laborDtTable = $("#laborDtTable").DataTable({
        ...
        "orderMulti": true,
        "ajax": {
            "url": "/Dashboard/PopulateLabor?workOrderId=" + workOrderID,
            "type": "POST",
            "datatype": "json",
...
function LaborAddOnSuccess(data) {
    if (data.Result == "success") {
        $(document).find('#AddLaborModalpopup').modal("hide");
        var message;
        if (data.mode == "add") {
...
function generatePartsListDataTable() {
    ...
    PartsTable = $("#partsListSearchTable").DataTable({
        ...
        "orderMulti": true,
        "ajax": {
            "url": "/Dashboard/GetPartListGrid?workOrderId=" + workOrderID,
            "type": "POST",
            "datatype": "json"
        },
        //V2-624
...
$(document).on('change', '#StoreroomId', function () {
    if ($('#StoreroomId').val() == '') {
        $(document).find('#AddCompletionWorkbenchPartIssueModal').find('#openpartgrid').hide();
        $(document).find('#AddCompletionWorkbenchPartIssueModal').find('#btnQrScanner').hide();
    }
    else {
        $(document).find('#AddCompletionWorkbenchPartIssueModal').find('#openpartgrid').show();
        $(document).find('#AddCompletionWorkbenchPartIssueModal').find('#btnQrScanner').show();
    }
    $(document).find('#txtpartid,#PartIssueAddModel_PartId').val('');
});
$(document).on('click', '#AddPartsIssue', function () {
    $.ajax({
        url: '/Dashboard/AddPartIssue',
        type: 'POST',
        dataType: 'html',
        beforeSend: function () {
            ShowLoader();
        ...
        complete: function () {
            if ($('#divAddParts').html() != '') {
                SetControls();
                $(document).find('#AddCompletionWorkbenchPartIssueModal').modal("show");
            }
            CloseLoader();
        },
        error: function () {
...
$(document).on('hidden.bs.modal', '#AddCompletionWorkbenchPartIssueModal', function () {
    $('#divAddParts').html('');
});
...
$(document).on('click', '.link_xrefpart_detail', function (e) {
    ...
    $(document).find("#maintenancepartIdModal").modal('hide');
});
...
function PartIssueAddOnSuccess(data) {
    $("#AddCompletionWorkbenchPartIssueModal").find("input,select").val('').end().modal("hide");
    if (data.Result == "success") {
...
function generateWoTaskGrid() {
    ...
    woTaskTable = $("#woTaskTable").DataTable({
        ...
        "orderMulti": true,
        "ajax": {
            "url": "/Dashboard/PopulateTask?workOrderId=" + workOrderID,
            "type": "POST",
            "datatype": "json",
            "dataSrc": function (response) {
...
$(document).on('click', '#select-all-wotask', function () {
    ...
    if (checked) {
        ...
        TaskIdsToupdate = [];
        $.ajax({
            "url": "/Dashboard/PopulateTaskIds?workOrderId=" + workOrderID,
            async: true,
            type: "GET",
            beforeSend: function () {
                ShowLoader();
...
function LoadCommentsTab() {
    var WorkOrderId = $(document).find('#WorkOrderId').val();
    $.ajax({
        url: '/Dashboard/GetCommentsDetails',
        type: 'POST',
        beforeSend: function () {
            ShowLoader();
        },
...
function generateMaintenanceCompletionAttachmentGrid() {
    ...
    maintenanceAttachmentTable = $("#MaintenanceAttachmentTable").DataTable({
        ...
        "orderMulti": true,
        "ajax": {
            "url": "/Dashboard/PopulateAttachment?workOrderId=" + workOrderID,
            "type": "POST",
            "datatype": "json",
            "dataSrc": function (response) {
...
$(document).on('click', '.lnk_attachment_download', function (e) {
    ...
    window.location = '/Dashboard/DownloadAttachment?_fileinfoId=' + data.FileAttachmentId;
});
//#endregion 
...
function CompleteWorkorderJob(WorkOrderId) {
    $.ajax({
        url: '/Dashboard/CompleteWorkOrder',
        data: {
            workorderId: WorkOrderId
        },
        type: "GET",
...
$(document).on('click', "#AddWorkRequestBtn", function (e) {
    var workOrderId = parseInt($(document).find('#WorkOrderId').val());
    $.ajax({
        url: "/Dashboard/AddWoRequestDynamicInMaintenanceCompletionWorkbench",
        type: "GET",
        dataType: 'html',
        //data: {
        //    'WorkOrderId': workOrderId,
...
function WorkRequestDynamicAddOnSuccess(data) {
    if (data.data === "success") {
        ...
        //        $(document).find("#wrEquipTreeModal").hide();
...
$(document).on('hidden.bs.modal', '#wrEquipTreeModal', function () {
...
$(document).on('click', "#BtnReturnPartsDashboard", function () {
    ...
    $.ajax({
        url: '/Dashboard/ReturnPartSelectedList',
        type: "POST",
        datatype: "json",
...
$(document).on('click', '#btnQrScanner', function () {
    ...
    if (!$(document).find('#QrCodeReaderModal').hasClass('show')) {
        ...
        StartQRReaderPartIssue();
    }
});
function StartQRReaderPartIssue() {
...
function LoadMaterialRequestTab() {
    $.ajax({
        url: '/Dashboard/LoadMaterialRequest',
        type: 'POST',
        data: {
            'WorkOrderId': $(document).find('#WorkOrderId').val()
        },
...
function generateMaintenanceCompletionMaterialRequestGrid() {
    ...
    dtEstimatesPartTable = $("#tblMaterialRequest").DataTable({
        ...
        "orderMulti": true,
        "ajax": {
            "url": "/Dashboard/PopulateMaterialRequest?workOrderId=" + workOrderID,
            "type": "POST",
            "datatype": "json",
            data: function (d) {
...
function GoToAddPartInInventory(WorkOrderID) {
    ...
    $.ajax({
        url: "/Dashboard/AddPartInInventory",
        type: "POST",
        dataType: "html",
...
function AddEstimatePartNotInInventory() {
    ...
    var clientLookupId = $(document).find('#WorkOrderClientLookupId').val();
    $.ajax({
        url: "/Dashboard/AddEstimatesPartNotInInventory",
        type: "GET",
        dataType: 'html',
        data: { WorkOrderId: workorderId, ClientLookupId: clientLookupId },
        beforeSend: function () {
...
function EstimatePartNotInInventoryAddOnSuccess(data) {
    ...
    if (data.Result == "success") {
        $('#AddPartNotInInventoryModalpopup').hide();
        if (data.mode == "add") {
...
$(document).on('click', '.editWoEstimatesPartBttn', function () {
    ...
    if (CategoryId > 0) {
        $.ajax({
            url: "/Dashboard/EditPartInInventory",
            type: "GET",
            dataType: 'html',
            data: {
        ...
        $.ajax({
            url: "/Dashboard/EditEstimatesPart",
            type: "GET",
            dataType: 'html',
...
$(document).on('click', '#spnlinkToSearch', function () {
...
$(document).on('click', '#lnk_psearchplupdashboard', function (e) {
    e.preventDefault();
    resetpage();
    var DashboardId = $('#DashboardlistingId').val();
    if (DashboardId == null || DashboardId == "") {
...
function generateDowntimeGrid() {
    ...
    downtimeDtTable = $("#downtimeDtTable").DataTable({
        ...
        "orderMulti": true,
        "ajax": {
            "url": "/Dashboard/PopulateDowntime?workOrderId=" + workOrderID,
            "type": "POST",
            "datatype": "json",
            "dataSrc": function (response) {
...
function DowntimeAddOnSuccess(data) {
    if (data.Result == "success") {
        $(document).find('#AddDowntimeModalpopup').modal("hide");
        var message;
        if (data.mode == "add") {
...
function SetControlsDownTime() {
    ...
    $(document).on('change', '#woDowntimeModel_ReasonForDown', function () {
        var tlen = $(document).find("#woDowntimeModel_ReasonForDown").val();
        if (tlen.length > 0) {
            var areaddescribedby = $(document).find("#woDowntimeModel_ReasonForDown").attr('aria-describedby');
            $('#' + areaddescribedby).hide();
            ...
            var areaddescribedby = $(document).find("#woDowntimeModel_ReasonForDown").attr('aria-describedby');
            $('#' + areaddescribedby).show();
            $(document).find('form').find("#woDowntimeModel_ReasonForDown").addClass("input-validation-error");
        }
    });
    SetFixedHeadStyle();
}
    ...
    new Compressor(files, {
        ...
        success(result) {
            if (result.size < files.size) {
                SaveCompressedImage(result, result.name.replace(/^.*[\\\/]/, ''));
            }
            else {
                SaveCompressedImage(files, files.name.replace(/^.*[\\\/]/, ''));
            }
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.result === "success") {
                ...
                $('.equipImg').attr('src', data.imageurl);
                //$('.zoomWindow').attr('style','background-image:url('+ data.imageurl+')');
                //$(document).find('#AzureImage').append('<a href="javascript:void(0)" id="deleteImg" class="trashIcon" title="Delete"><i class="fa fa-trash"></i></a>');
                $('#EquipZoom').data('zoomImage', data.imageurl).elevateZoom(
                    {
                        zoomType: "window",
...
function GoToSendMRForApproval() {
    var WorkOrderId = $(document).find('#WorkOrderId').val();
    $.ajax({
        url: "/Dashboard/SendMRForApproval",
        type: "GET",
        dataType: 'html',
        data: { WorkOrderId: WorkOrderId },
        beforeSend: function () {
...
function PopulateStorerooms() {
    ...
    $.ajax({
        url: "/Dashboard/PopulateStorerooms",
        type: "GET",
        dataType: 'html',
...
$(document).on('click', "#AddSanitationRequestBtn", function (e) {
    ...
    var clientLookupId = $(document).find('#WorkOrderClientLookupId').val();
    $.ajax({
        url: "/Dashboard/AddSanitationRequestWO",
        type: "GET",
        dataType: 'html',
        data: {
            'WorkoderId': workOrderId, 'ClientLookupId': clientLookupId
        ...
        success: function (data) {
            ...
            $('#AddSanitationRequestWODashboardModalpopup').modal({ backdrop: 'static', keyboard: false, show: true });
        },
        complete: function () {
            SetControls();
        },
...
$(document).on('click', '#btnCancelAddSanitationRequestWODashboard', function () {
    $("#AddSanitationRequestWODashboardModalpopup").modal('hide');
});
$(document).on('click', '#btnCancelAddSanitationRequestWO', function () {
    $("#AddSanitationRequestWODashboardModalpopup").modal('hide');
});
$(document).on('hidden.bs.modal', '#AddSanitationRequestWODashboardModalpopup', function () {
...
//#region V2-1067
$(document).on('hidden.bs.modal', '#AddDescribeWOModalpopupDynamic', function () {
    $('#DescribeWOPopUpDynamic').html('');
});
$(document).on('click', '.btnCancelDescribeDynamic', function () {
    $("#AddDescribeWOModalpopupDynamic").modal('hide');
    //StatusArray = [];
    var areaChargeToId = "";
