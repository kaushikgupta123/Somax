...
$(document).on('click', '.scrollview-equipmentsearch', function (e) {
    ...
    if ($(document).find('#txtChargeToId,#txtChargeTo').length > 0) {
        ...
        $(document).find('#ODescribeModel_PlantLocationDescription').val(Name).trigger('mbsc-enhance');
    }
    else {
        $(document).find('#' + Equipment_textFieldID_Mobile_WR).val($(this).data('clientlookupid')).trigger('mbsc-enhance');//.removeClass('input-validation-error');
        $(document).find('#' + Equipment_valueFieldID_Mobile_WR).val($(this).data('id'));//.removeClass('input-validation-error');

        $(document).find('#' + Equipment_textFieldID_Mobile_WR).closest('form').valid();

        $(document).find('#' + Equipment_valueFieldID_Mobile_WR).parent().find('div > button.ClearAccountModalPopupGridData').css('display', 'block');

        if ($(document).find('#' + Equipment_textFieldID_Mobile_WR).parents('div').eq(0).css('display') == 'none') {
            $(document).find('#' + Equipment_textFieldID_Mobile_WR).parents('div').eq(0).css('display', 'block');
        }
        if ($(document).find('#' + Equipment_valueFieldID_Mobile_WR).parents('div').eq(0).css('display') == 'block') {
            $(document).find('#' + Equipment_valueFieldID_Mobile_WR).parents('div').eq(0).css('display', 'none');
        }
    }
...
$(document).on('click', '#SJEquipmentModalHide', function () {
    ...
    $('#txtEquipmentWRSSearch_Mobile').val('');
});
$(document).on("keyup", '#txtEquipmentWRSSearch_Mobile', function (e) {
    if (e.keyCode == 13) {
...
function InitializeAssetListView_Mobile() {
    ...
    $('#SJEquipmentWOModal').addClass('slide-active');
}
