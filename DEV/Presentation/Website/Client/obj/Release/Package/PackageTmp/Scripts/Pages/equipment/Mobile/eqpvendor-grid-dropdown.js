...
$(document).on('click', '.scrollview-vendorsearch', function (e) {
    ...
    $(document).find('#' + TextField).val($(this).data('clientlookupid') + "-" + $(this).data('name')).trigger('mbsc-enhance');//.removeClass('input-validation-error');
    ...
    //$(document).find("#VendorTableModalPopupHide").trigger('click');
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i = 0; i < data.data.length; ++i) {
                item = data.data[i];
                lihtml = '';
                lihtml = '<li class="scrollview-vendorsearch" data-id="' + item.VendorID + '" data-clientlookupid="' + item.ClientLookupId + '" data-name="' + item.Name + '">';
