...
var WOCompletionWizard = function () {
    ...
    var initWizard = function () {
        ...
        wizard.on('beforeNext', function (wizard) {
            ...
            //}
            //if ($('#m-wizard__form-step--current').eq(0).attr('id') == 'CompletionInformationStep') {
...
$(document).on('click', '.btnWizardLaborcancel', function () {
    //$('#WorkorderCompletionLaborModalHide').trigger('click');
    $('#WorkorderCompletionLaborModal').removeClass('slide-active');
...
$(document).on('click', "#AddWizardLaborbtn , .addLaborBttnWizard", function (e) {
    editIndex = -1;
    $.ajax({
        url: "/Dashboard/AddLaborFromCompletionWizard_Mobile",
        type: "POST",
        dataType: 'html',
        beforeSend: function () {
            ShowLoader();
        ...
        complete: function () {
            ...
            if (globalPageType == "CompletionWorkbench") {
                LaborMobiscrollControll();
            }
            //$('#WorkorderCompletionLaborModal').modal({ backdrop: 'static', keyboard: false, show: true });
            $('#WorkorderCompletionLaborModal').addClass('slide-active').trigger('mbsc-enhance');
        },
        error: function () {
            CloseLoader();
        }
...
function LaborMobiscrollControll() {
    ...
    $('#CompletionLaborWizard_StartDate').mobiscroll().calendar({
        display: 'bottom',
        touchUi: true,
      /*  weekDays: 'min',*/
        yearChange: false,
    ...
    $(document).on("change", "#CompletionLaborWizard_StartDate", function (e) {
        ValidatePast30DaysDateFromDatePicker($(this).val(), getResourceValue("ValidatePast30DaysDateFromDatePickerAlert"));
    });
    $('#CompletionLaborWizard_Hours').mobiscroll().numpad({
        //touchUi: true,
        min: 0.01,
        max: 23.99,
        scale: 2,
    ...
    var x = parseFloat($('#CompletionLaborWizard_Hours').val()) == 0 ? '' : $('#CompletionLaborWizard_Hours').val();
    $('#CompletionLaborWizard_Hours').mobiscroll('setVal', x);
...
function LoadWorkOrderCompletionWizardModal(divElement, WorkOrderIds, PageType) {
    $.ajax({
        url: "/Dashboard/CompleteWorkOrderFromWizard_Mobile",
        type: "POST",
        ...
        complete: function () {
            ...
            BindMobiscrollControlForWOCompletionWizardTab();
...
$(document).on('click', '#WorkOrderCompletionWizardModalHide', function (e) {
    $('#WorkorderCompletionWizardDiv').find('.slide-modal').removeClass("slide-active");
    if ($('#CommentEditorFromWizard').length > 0) {
...
//$(document).on('click', '.complete', function () {  //Debashis Bose complete-modal
//    $('#WorkorderCompletionWizardDiv').find('.slide-modal').addClass("slide-active");

//});

//$(document).on('click', '.complete-slide-back-btn', function () {  //Debashis Bose complete-modal
//    $('#WorkorderCompletionWizardDiv').find('.slide-modal').removeClass("slide-active");
//});
...
$(document).on('click', '#CompleteWoCheckListWizard_Workbench,#actionCompleteWoCheckListWizard_Workbench', function () {
    var WorkOrderId = $(document).find('#WorkOrderId').val();
...
function BindMobiscrollControlForWOCompletionWizardTab() {
    $('#WorkorderCompletionWizardDiv').find('.slide-modal').addClass("slide-active");
    $('#WorkorderCompletionWizardDiv').trigger('mbsc-enhance');
    var maxVal, precision;
    $(document).find('.decimalinput').each(function () {
        maxVal = $(this).data('val-range-max'), precision = 6;
        if (maxVal == undefined || maxVal == null) {
            maxVal = 99999999.999999;
        }
        else {
            var splitData = (maxVal + '').split('.');
            if (splitData.length == 2) {
                precision = splitData[1].length;
            }
        }
        ...
        var x = parseFloat($(this).val()) == 0 ? '' : $(this).val();
        $(this).mobiscroll('setVal', x);
    });
}
