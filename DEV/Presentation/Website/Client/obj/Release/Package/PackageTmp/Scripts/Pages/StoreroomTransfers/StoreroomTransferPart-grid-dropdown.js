...
$(document).on('click', '#openpartgrid', function () {
    Part_Storeroomid = $(this).data('multistoreroomvalue');
...
function generatePartsXrefDataTable() {
    ...
    dtXrefPartsTable = $("#XrefPartsTable").DataTable({
        ...
        initComplete: function () {
            ...
            SetPageLengthMenu();
            if (!$(document).find('#PartStoreroomTransferModal').hasClass('show')) {
                $(document).find('#PartStoreroomTransferModal').modal("show");
            }
...
$(document).ready(function () {
    ...
    //$(document).find('#ErrorResult div').html('');
    //$(document).find('#ErrorResult').css('border', 'none');
});
$(document).on('click', '.link_xrefpart_detail', function (e) {
    ...
    var data = dtXrefPartsTable.row(row).data();
        ...
        getIssuedropdown(data.PartId, Part_Storeroomid)
   
    $(document).find("#PartStoreroomTransferModal").modal('hide');
});

function getIssuedropdown(PartId, Part_Storeroomid) {
    $.ajax({
        url: "/StoreroomTransfers/GetIssueStoreroomList",
        type: "GET",
        datatype: "json",
        data: {
            'PartId': PartId,
            'RequestStoreroomId': Part_Storeroomid
        },
        beforeSend: function () {
            ShowLoader();
        },
        success: function (data) {
            $('#addTransferRequest_IssuePartStoreroomIdAndStoreroomId').html("");
            var select = "";
            if (data.StoreroomList.length > 1) {
                select = '<option  value="">--Select--</option>';
            }
           
            for (var i = 0; i < data.StoreroomList.length; i++) {
                select = select + '<option value="' + data.StoreroomList[i].Value + '">' + data.StoreroomList[i].Text + '</option>'

            }
            $('#addTransferRequest_IssuePartStoreroomIdAndStoreroomId').html(select);
            $(document).find('#addTransferRequest_IssuePartStoreroomIdAndStoreroomId').select2({});
            CloseLoader();
        },
        complete: function () {
            CloseLoader();
