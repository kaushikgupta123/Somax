...
function GeneratePartLookUpGrid() {
    ...
    partLokkUpdt = $("#tblpartlookup").DataTable({
        ...
        "orderMulti": true,
        "ajax": {
            "url": "/PartLookup/GetPartLookUpGridWO",
            "type": "post",
            "datatype": "json",
            data: function (d) {
                    ...
                    mRender: function (data, type, full, meta) {
                        ...
                        if (full.InVendorCatalog) {
                            if (getIndexIfObjWithAttr(tablecarItemarr, 'PartId', myNewdata, 'Indexid', myIndexdata) != -1) {
                                ...
                                if (ShoppingCart && ShoppingCart === "True" && (WorkOrderID === "0" || MaterialRequestId === "0")) {
...
function ShowCardViewWO() {
    ...
    var sorttext = '';
    $.ajax({
        url: '/PartLookup/GetCardViewDataWO',
        type: 'POST',
        data: {
...
function ShowCardViewWO_FromDashboard(WorkOrderID) {
    ...
    var ModeForRedirect = ($('#ModeForRedirect').val() == null || $('#ModeForRedirect').val() == undefined) ? 'Dashboard' : $('#ModeForRedirect').val();
    if (!orderbycol) {
        orderbycol = 1;
    }
...
function ClickMeItem(r) {
    if ($(r).is(":checked")) {
        ...
        var tblStoreroomId = $(r).closest('tr').children('td').find('.hidtableStoreroomId').val();
        var tblPartStoreroomId = $(r).closest('tr').children('td').find('.hidtablePartStoreroomId').val();
...
function FillAddtoCartSide(_data, IsDivExists, PartId, Indexid) {
    $.ajax({
        url: '/PartLookup/GetAddToCartDataWO',
        type: 'POST',
        data: _data,
        success: function (data) {
            if (IsDivExists) {
...
function FillAllAddtoCartSide(_data) {
    $.ajax({
        url: '/PartLookup/GetAllAddToCartDataWO',
        type: 'POST',
...
function AddtoCartItem(rItem) {
    ...
    if (IsCardView && $(rItem).attr('data-AddmyCart-Id') == 'btn-AddmyCart-Id') {
        ...
        /* var PartDesc = $(rItem).siblings('.clsPartDesc').text();*/
        ...
        var tblRequiredDate = $(rItem).siblings('.requiredBox').find('.requiredDate').val();
        //V2-665 
        ...
        //V2-690
        if (tblRequiredDate === undefined || tblRequiredDate === "") {
            tblRequiredDate = null;
        }
        ...
        if (parseFloat(tblitemQty) > parseFloat(tblQtyMaximum)) {
            swal({
                ...
                function () {
                    ...
                    // $('.' + tblPartId + '.' + tblIndexid).prop('checked', true);
                    selectedcount = selectedcount + tablecarItemarr.length;
                    $('.chkitemcount').text(tablecarItemarr.length);
                    if (tablecarItemarr.length > 0) {
                        $(".updateArea").fadeIn();
...
function ProcessCartData() {
    ...
    $('.sidebarCartList').each(function (index, value) {
        ...
        /* var Description = $(this).find('.dtlsBox').children('p').text();*/
...
$(document).on('click', '#btnProcessCartData,#btnprocess', function () {
    ...
    var WorkOrderID = $('#WorkOrderID').val();
    var MaterialRequestId = $('#MaterialRequestId').val();
    var PreventiveMaintainId = $('#PreventiveMaintainId').val();
    var cartDataLength = $('.sidebarCartList').length;
    var ModeForRedirect = $('#ModeForRedirect').val();
    if (cartDataLength > 0) {
        ProcessCartData();
        var _mypartdata = JSON.stringify({
            ...
            PreventiveMaintainId: PreventiveMaintainId
        });
        var message = '';
        ...
        if (myQuantityEmptyProductarr.length != 0) {
            ...
            if (myQuantityZeroProductarr.length == 0) {
                $.ajax({
                    url: '/PartLookup/ProcesssPartCartDataWO',
                    contentType: "application/json",
                    type: 'POST',
                    data: _mypartdata,
                    beforeSend: function () {
                        ...
                        if (data.errormessge != "") {
                            ...
                            if (WorkOrderID != 0) {
                                ...
                                if (data.status != 'fail') {
                                    ...
                                    CloseLoader();
                                    swal(SuccessAlertSetting, function () {
                                        RedirectToPmDetail(PreventiveMaintainId, "estimatesPart");
                                        resetpage();
                                    });
                                }
                            }
...
$(document).on('click', '#lnk_psearchdtls', function (e) {
    ...
    var WorkOrderID = $('#WorkOrderID').val();
    var MaterialRequestId = $('#MaterialRequestId').val();
    var PreventiveMaintainsId = $('#PreventiveMaintainId').val();
    if (WorkOrderID != 0) {
        RedirectToPmDetail(WorkOrderID, "estimatespart");
    }
    else if (MaterialRequestId != 0) {
    ...
    else if (PreventiveMaintainsId != 0) {
        RedirectToPmDetail(PreventiveMaintainsId,"estimatesPart")
    }
});
...
$(document).on('click', '#lnk_psearchplup', function (e) {
    ...
    var WorkOrderID = $('#WorkOrderID').val();
    var MaterialRequestId = $('#MaterialRequestId').val();
    var PreventiveMaintId = $('#PreventiveMaintainId').val();
    if (WorkOrderID != 0) {
        location.href = "/WorkOrder/Index?page=Maintenance_Work_Order_Search";
    }
    else if (MaterialRequestId != 0) {
        location.href = "/MaterialRequest/Index?page=Inventory_MaterialRequest";
    }
    else if (PreventiveMaintId != 0) {
        location.href = "/PreventiveMaintenance/Index?page=Maintenance_Preventive_Maintenance_Search";
    }
});
//#endregion
...
$(document).on('click', "#sidebarCollapsePartLookupWO", function () {
    var ModeForRedirect = $('#ModeForRedirect').val();
    if (ModeForRedirect == 'Dashboard') {
    ...
    $.ajax({
        ...
        "success": function (json) {
            ...
            if (json.LayoutInfo !== '') {
                ...
                [orderbycol, orderDir] = gridstate.order[0];
