...
function generateWorkOrderDataTable() {
    ...
    var rCount = 0;
    var visibility;
    if ($(document).find('#WorkOrderPITable').hasClass('dataTable')) {
        dtWorkOrderTable.destroy();
    }
    mcxDialog.loading({ src: "../content/Images" });
    dtWorkOrderTable = $("#WorkOrderPITable").DataTable({
        searching: true,
        serverSide: true,
        ...
        "orderMulti": true,
        "ajax": {
            "url": "/PartIssue/GetWorkOrderLookupListChunkSearchForPartsIssue",
            data: function (d) {
        ...
        initComplete: function () {
            $(document).find('#tblWorkOrderPIfooter').show();
            mcxDialog.closeLoading();
            SetPageLengthMenu();
            if (!$(document).find('#WorkOrderPIModal').hasClass('show')) {
                $(document).find('#WorkOrderPIModal').modal("show");
            }
            $(document).find('#WorkOrderPITable tfoot th').each(function (i, v) {
                var colIndex = i;
            ...
            $('#WorkOrderPITable tfoot th').find('.chkwo').on("keyup", function (e) {
                if (e.keyCode == 13) {
...
$(document).on('click', '.link_CHKWorkOrder', function (e) {
    var index_row = $('#WorkOrderPITable tr').index($(this).closest('tr')) - 1;
    ...
    $(document).find("#WorkOrderPIModal").modal('hide');
});