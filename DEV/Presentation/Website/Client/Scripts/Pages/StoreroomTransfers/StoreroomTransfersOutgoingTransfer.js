...
var gridname = "StoreroomTransfers_OutgoingTransfer_Search";
...
function funcOutgoingStoreroomProcess(StoreroomTransferId, IssueQty) {
...
$(function () {
    ...
    generateOutgoingStoreroomTransferDataTable();
});
var order = '0';
var orderDir = 'asc';
var IsIssue = false;
function generateOutgoingStoreroomTransferDataTable() {
    var printCounter = 0;
    if ($(document).find('#StoreroomTransferOutgoingSearchTable').hasClass('dataTable')) {
        dtTable.destroy();
    }
    dtTable = $("#StoreroomTransferOutgoingSearchTable").DataTable({
        colReorder: {
            fixedColumnsLeft: 2
        },
        rowGrouping: true,
        ...
        "orderMulti": true,
        "ajax": {
            "url": "/StoreroomTransfers/GetStoreroomTransferOutgoingTransferGridData",
            "type": "post",
            "datatype": "json",
            data: function (d) {
        ...
        "columns":
            [
                ...
                {
                    "data": "PartDescription", "autoWidth": false, "bSearchable": true, "bSortable": true, "name": "2",
                    "mRender": function (data, type, row) {
                        return "<div class='text-wrap width-300'>" + data + "</div>";
                    }
                },
                { "data": "RequestQuantity", "autoWidth": true, "bSearchable": true, "bSortable": true, "className": "text-right", "name": "3", },
                {
                ...
                {
                    "data": "QuantityIssued", "autoWidth": true, "bSearchable": true, "bSortable": true, "className": "text-right", "name": "6"
                },
                {
                    "data": "StoreroomName", "autoWidth": true, "bSearchable": true, "bSortable": true, "name": "7"
                },
                {
                    "data": "StoreroomTransferId", "autoWidth": true, "bSearchable": true, "className": "text-center", "bSortable": false, "name": "8",
                    'render': function (data, type, row) {
                        if (IsIssue === true) {
                            var DenyTransferRequest = "";
                            if (row.Status === 'Open') {
                                DenyTransferRequest = '<a class="btn btn-blue DenyTransferRequestBtn gridinnerbutton" style="margin-left: 0;" title= "Deny Transfer Request">Deny Transfer Request</a>';
        ...
        initComplete: function (settings, json) {
            ...
            DisableExportButton($("#StoreroomTransferOutgoingSearchTable"), $(document).find('.import-export'));
            CloseLoader();
        }
    });
}
...
$(document).on('click', '#StoreroomTransferOutgoingSearchTable_paginate .paginate_button', function () {
    run = true;
});
$(document).on('change', '#StoreroomTransferOutgoingSearchTable_length .searchdt-menu', function () {
    run = true;
});
...
////#endregion
$('#StoreroomTransferOutgoingSearchTable').find('th').click(function () {
    run = true;
    order = $(this).data('col');
});
...
$(document).on('click', "#SrchBttnNew", function () {
    $.ajax({
        ...
        type: 'GET',
        data: { tableName: 'OutgoingStoreroomTransfer' },
        beforeSend: function () {
...
function GenerateSearchList(txtSearchval, isClear) {
    ...
    $.ajax({
        ...
        type: 'POST',
        data: { tableName: 'OutgoingStoreroomTransfer', searchText: txtSearchval, isClear: isClear },
        beforeSend: function () {
            ShowLoader();
        },
        success: function (data) {
...
$(document).on('change', '.grd-hours', function () {
    ...
    if (ProcessSelectedItemArray.length > 0 && exist.length > 0) {
        ...
        if (IssueQty == "" || IssueQty == "0") {
            ...
            ProcessSelectedItemArray[index].IssueQty = IssueQty;
        }
    }
    else {
        var thisApproval = new funcOutgoingStoreroomProcess(StoreroomTransferId, IssueQty);
...
$(document).on('click', "#btnProcess", function () {
    ...
    $.each(STData, function (index, item) {
        if (item.IssueQty != "" && isNaN(item.IssueQty) == true) {
            validqty = false;
        }
    });
    if (validqty == false) {
        ShowErrorAlert("Please enter valid Issue Qty.");
        return false;
    }
    STData = JSON.stringify({ 'STData': STData });
    $.ajax({
        url: '/StoreroomTransfers/ProcessOutgoingStoreroomTransfers',
        data: STData,
        ...
        success: function (data) {
            if (data.Result == "success") {
                ...
                $('.grd-hours').val('');
                SuccessAlertSetting.text = getResourceValue("AlertIssueSuccess");
...
$(document).on('click', '.DenyTransferRequestBtn', function (e) {
    var row = $(this).parents('tr');
    var data = dtTable.row(row).data();
    CancelAlertSetting.text = getResourceValue("ConfirmDenyTransferRequest");
    swal(CancelAlertSetting, function () {
        $.ajax({
            url: "/StoreroomTransfers/DenyTransferRequest",
            data: {
            ...
            success: function (data) {
                if (data.Result == "success") {
                    SuccessAlertSetting.text = getResourceValue("AlertDenyTransferRequest");
