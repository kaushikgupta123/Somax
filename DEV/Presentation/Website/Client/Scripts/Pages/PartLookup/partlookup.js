...
var actionvendorcatlogstrs = '<input type="text" data-Qty-Id="tablepartQtyCard" data-PartId="#valPartId" data-indexid="#valIndexId" class="form-control search qtyothertext qtytableItemQty allownumericWithdecimal" onkeyup="getCurQty(this)" value="#valQty" maxlength="8"  placeholder="Qty">' +
    ...
    '#RequiredHtmlContent' +
    '<button type="button" class="btn btn-danger btn-cart btn-addToCartTable clsnameOnorder" id="#ValbtnidName" data-AddtoCart-Id="btn-MyaddToCartTable" title="Add to Cart"><i class="fa fa-shopping-cart"></i></button>';
...
function GeneratePartLookUpGrid() {    
    ...
    partLokkUpdt = $("#tblpartlookup").DataTable({
        ...
        "orderMulti": true,
        "ajax": {
            "url": "/PartLookup/GetPartLookUpGrid",
            "type": "post",
            "datatype": "json",
            data: function (d) {
                    ...
                    mRender: function (data, type, full, meta) {
                        ...
                        if (full.InVendorCatalog) {
                            if (getIndexIfObjWithAttr(tablecarItemarr, 'PartId', myNewdata, 'Indexid', myIndexdata) != -1) {
                                ...
                                return actionvendorcatlogstr.replace('#val0', tablecarItemarr[index].UnitCost).replace('#val1', tablecarItemarr[index].UnitCost).replace('#val2', tablecarItemarr[index].UnitofMeasure).replace('#valQty', tablecarItemarr[index].OrderQuantity).replace('#valPartId', full.PartId).replace('#valPartIdNew', full.PartId).replace('#RequiredHtmlContent', ReplaceRequiredHtmlContent).replace('#valIndexId', full.indexid).replace('#ValbtnidName', 'btnaddToCartTable_' + full.PartId+'_'+full.indexid);
                            }
                            else {
                                if (ShoppingCart && ShoppingCart === "True" && PurchaseOrderId === "0") {
                                    ReplaceRequiredHtmlContent = RequiredHtmlContent.replace('#valRequiredDate', full.RequiredDate);
                                }
                                return actionvendorcatlogstr.replace('#val0', full.Price).replace('#val1', full.Price).replace('#val2', full.Unit).replace('#valQty', 1).replace('#valPartId', full.PartId).replace('#valPartIdNew', full.PartId).replace('#RequiredHtmlContent', ReplaceRequiredHtmlContent).replace('#valIndexId', full.indexid).replace('#ValbtnidName', 'btnaddToCartTable_' + full.PartId + '_' + full.indexid).replace('#ValbtnidName', 'btnaddToCartTable_' + full.PartId + '_' + full.indexid);
                            }
                        }
                        else {
                            if (getIndexIfObjWithAttr(tablecarItemarr, 'PartId', myNewdata, 'Indexid', myIndexdata) != -1) {
                                ...
                                {                                    
                                    return actionnotvcstr.replace('#val0', tablecarItemarr[mindex].UnitCost).replace('#val1', tablecarItemarr[mindex].UnitCost).replace('#val2', tablecarItemarr[mindex].UnitofMeasure).replace('#valQty', '').replace('#valPartId', full.PartId).replace('#valPartIdNew', full.PartId).replace('#RequiredHtmlContent', ReplaceRequiredHtmlContent).replace('#valIndexId', full.indexid).replace('#ValbtnidName', 'btnaddToCartTable_' + full.PartId + '_' + full.indexid);
                                }
                                else {
                                    if (full.UOMConvRequired) {
                                        return actionvendorcatlogstr.replace('#val0', tablecarItemarr[mindex].UnitCost).replace('#val1', tablecarItemarr[mindex].UnitCost).replace('#val2', tablecarItemarr[mindex].UnitofMeasure).replace('#valQty', tablecarItemarr[mindex].OrderQuantity).replace('#valPartId', full.PartId).replace('#valPartIdNew', full.PartId).replace('#RequiredHtmlContent', ReplaceRequiredHtmlContent).replace('#valIndexId', full.indexid).replace('#ValbtnidName', 'btnaddToCartTable_' + full.PartId + '_' + full.indexid);
                                    }
                                    else {
                                        return actionnotvcstr.replace('#val0', tablecarItemarr[mindex].UnitCost).replace('#val1', tablecarItemarr[mindex].UnitCost).replace('#val2', tablecarItemarr[mindex].UnitofMeasure).replace('#valQty', tablecarItemarr[mindex].OrderQuantity).replace('#valPartId', full.PartId).replace('#valPartIdNew', full.PartId).replace('#RequiredHtmlContent', ReplaceRequiredHtmlContent).replace('#valIndexId', full.indexid).replace('#ValbtnidName', 'btnaddToCartTable_' + full.PartId + '_' + full.indexid);
                                    }
                                ...
                                if (full.UOMConvRequired) {
                                    return actionvendorcatlogstr.replace('#val0', full.Price).replace('#val1', full.Price).replace('#val2', full.Unit).replace('#valQty', 1).replace('#valPartId', full.PartId).replace('#valPartIdNew', full.PartId).replace('#RequiredHtmlContent', ReplaceRequiredHtmlContent).replace('#valIndexId', full.indexid).replace('#ValbtnidName', 'btnaddToCartTable_' + full.PartId + '_' + full.indexid);
                                }
                                else {
...
function ShowCardView() { 
    ...
    var sorttext = '';
    $.ajax({
        url: '/PartLookup/GetCardViewData',
        type: 'POST',
        data: {
...
function FillAddtoCartSide(_data, IsDivExists, PartId, Indexid) {
    $.ajax({
        url: '/PartLookup/GetAddToCartData',
        type: 'POST',
        data: _data,
        success: function (data) {
            if (IsDivExists) {
...
function FillAllAddtoCartSide(_data) {
    $.ajax({
        url: '/PartLookup/GetAllAddToCartData',
        type: 'POST',
...
function AddAllToCartOnOrderCheck() {
    if (lengAlladdCard > 0 && $("#tableView").is(":visible") == true) {
        ...
        if (lengAlladdCard != lengAlladdcardDone && lengAlladdCard != 0) {
            ...
            ////deletion
            //for (var l = 0, li = NotAddedPartsLists.length; l < li; l++) {
            //    var ids = NotAddedPartsLists[l].Indexid;
            //    var mei = tablecarItemarr.findIndex(x => x.Indexid === ids);
            //    var tblMyPartId = tablecarItemarr[mei].PartId;
            //    var tblIndexId = tablecarItemarr[mei].Indexid;
            //    var classnew = '.chkcheckItem' + '.' + tblMyPartId + '.' + tblIndexId + '.index' + '_' + tblMyPartId + '_' + tblIndexId;
            //    if ($(classnew).is(':checked')) {
            //        $(classnew).prop('checked', false);
            //    }
            //    tablecarItemarr.splice(mei, 1);
            //}
            //$('.chkitemcount').text(tablecarItemarr.length);
            //NotAddedPartsLists = [];
            ////delete
...
/*var NotAddedPartsLists = [];*/
$(document).on('click', '#btncancelfromLineItemPopUp', function () {
   
    if (lengAlladdCard > 0 && $("#tableView").is(":visible") == true) {
        ////remove
        //var tblIndexId = tablecarItemarr[lengAlladdcardDone].Indexid;
        //NotAddedPartsLists.push({ Indexid: tblIndexId });
        ////remove
        var tblIndexId = tablecarItemarr[lengAlladdcardDone].Indexid;
...
$(document).on('click', '#btnProcessCartData,#btnprocess', function () {
    ...
    if (cartDataLength > 0) {
        ...
        if (myQuantityZeroProductarr.length == 0) {
            $.ajax({
                url: '/PartLookup/ProcesssPartCartData',
                contentType: "application/json",
                type: 'POST',
                data: _mypartdata,
                beforeSend: function () {
