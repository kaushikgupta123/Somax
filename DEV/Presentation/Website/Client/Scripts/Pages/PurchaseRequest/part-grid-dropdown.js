...
$(document).on('click', '.openpartgrid', function () {
    PartIndex= $(this).data('td-row');
    generatePRPartsXrefDataTable();
});
function generatePRPartsXrefDataTable() {
            ...
            [
                {
                    ...
                    "mRender": function (data, type, row) {
                        return '<a class="link_PR_xrefpart_detail" data-part_issue_unit="' + row.IssueUnit +'" href="javascript:void(0)">' + data + '</a>';
                    }
                },
                {
...
$(document).on('click', '.link_PR_xrefpart_detail', function (e) {
    var index_row = $('#XrefPartsTable tr').index($(this).closest('tr')) - 1;
    var row = $(this).parents('tr');
    var td = $(this).parents('tr').find('td');
    var data = dtXrefPartsTable.row(row).data();
    $(document).find('#PartId_' + PartIndex).val(data.PartId);
    $(document).find('#PartClientLookup_' + PartIndex).val(data.ClientLookUpId).removeClass('input-validation-error');
    $(document).find('.select-chargetype').eq(PartIndex).val('0').trigger('change.select2');
    $(document).find('.chargetolookup').eq(PartIndex).val('');
    $(document).find('.chargetolookupval').eq(PartIndex).val('');
    $(document).find("#partModal").modal('hide');
    $(document).find('.select-OrderUnit').eq(PartIndex).val($(this).data('part_issue_unit')).trigger('change.select2');//V2-1119
    PartIndex = -1;
});