...
function generateFleetFuelTable() {
    ...
    equipTable = $("#EquipPopupTable").DataTable({
        ...
        initComplete: function () {
            ...
            SetPageLengthMenu();
            if (!$(document).find('#FleetIssueModal').hasClass('show')) {
                $(document).find('#FleetIssueModal').modal("show");
            }
...
$(document).on('click', '.link_Issueeqp_detail', function (e) {
    ...
    $(document).find('#FleetIssueModel_ClientLookupId').val(data.ClientLookupId).removeClass('input-validation-error');   
    $(document).find("#FleetIssueModal").modal('hide'); 
