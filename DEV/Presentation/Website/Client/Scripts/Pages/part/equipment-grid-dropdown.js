...
$(document).on('click', '#opengrid', function () {
    generatePartsEquipmentTable();
});
function generatePartsEquipmentTable() {
        ...
        [
            {
                ...
                "mRender": function (data, type, row) {
                    return '<a class=link_parteqp_detail href="javascript:void(0)">' + data + '</a>'
                }
            },
...
$(document).on('click', '.link_parteqp_detail', function (e) {
    var index_row = $('#EquipPopupTable tr').index($(this).closest('tr')) - 1;
    var row = $(this).parents('tr');
    var td = $(this).parents('tr').find('td');
    var data = equipTable.row(row).data();
    $(document).find('#equipmentPartXrefModel_Equipment_ClientLookupId').val(data.ClientLookupId).removeClass('input-validation-error');
