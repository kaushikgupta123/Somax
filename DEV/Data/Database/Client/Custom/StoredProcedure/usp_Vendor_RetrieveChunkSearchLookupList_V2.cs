...
{
    public class usp_Vendor_RetrieveChunkSearchLookupList_V2
    {
        private static string STOREDPROCEDURE_NAME = "usp_Vendor_RetrieveChunkSearchLookupList_V2";
        public usp_Vendor_RetrieveChunkSearchLookupList_V2()
        {

        }

        public static List<b_Vendor> CallStoredProcedure(SqlCommand command, long callerUserInfoId,
                                         string callerUserName, b_Vendor obj)
        {
            List<b_Vendor> records = new List<b_Vendor>();
            b_Vendor b_Vendor = new b_Vendor();
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    b_Vendor = b_Vendor.ProcessRowForChunkSearchLookupList(reader);
                    records.Add(b_Vendor);
                }
            }
            catch (Exception ex)
            {
