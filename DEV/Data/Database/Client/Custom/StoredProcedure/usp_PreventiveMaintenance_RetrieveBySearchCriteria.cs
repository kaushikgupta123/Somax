...
{
    /// <summary>
    /// Access the usp_PreventiveMaintenance_RetrieveBySearchCriteria stored procedure.
    /// </summary>
    public class usp_PreventiveMaintenance_RetrieveBySearchCriteria
    {
        /// <summary>
        /// Constants.
        /// </summary>
        private static string STOREDPROCEDURE_NAME = "usp_PreventiveMaintenance_RetrieveBySearchCriteria";

        /// <summary>
        /// Default constructor.
        /// </summary>
        public usp_PreventiveMaintenance_RetrieveBySearchCriteria()
        {
        }
        ...
        public static List<KeyValuePair<b_PrevMaintMaster, b_PrevMaintSched>> CallStoredProcedure(
            SqlCommand command,
        ...
        {
            List<KeyValuePair<b_PrevMaintMaster, b_PrevMaintSched>> records = new List<KeyValuePair<b_PrevMaintMaster, b_PrevMaintSched>>();
            SqlDataReader reader = null;
            SqlParameter RETURN_CODE_parameter = null;

            b_PrevMaintMaster key;
            b_PrevMaintSched val;
            ...
            command.SetStringInputParameter(SqlDbType.VarChar, "ChargeType", chargeType, 256);
            command.SetStringInputParameter(SqlDbType.VarChar, "ScheduleType", scheduleType, 256);
            command.SetStringInputParameter(SqlDbType.VarChar, "Crew", crew, 256);
            command.SetStringInputParameter(SqlDbType.VarChar, "Priority", priority, 256);
            command.SetStringInputParameter(SqlDbType.VarChar, "Shift", shift, 256);
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    key = (b_PrevMaintMaster)b_PrevMaintMaster.ProcessRow(reader);
                    val = (b_PrevMaintSched)b_PrevMaintSched.ProcessRowForPrevMaintComposite(reader);
                    records.Add(new KeyValuePair<b_PrevMaintMaster, b_PrevMaintSched>(key, val));
                    temp = key.ClientLookupId;
